...
//</Setting>";
        ...
        {
            strout = CheckRegistryKey(path, gameName);
            if (!string.IsNullOrEmpty(strout)) return;

            strout = CheckRegistryKey(path, gameNameSteam);
            if (!string.IsNullOrEmpty(strout)) return;
        ...
        private static string CheckRegistryKey(string path, string gameName)
        {
            try
            {
                var subKey = Registry.CurrentUser.OpenSubKey($@"Software\ILLGAMES\{gameName}");
                if (subKey != null)
                {
...
";
        ...
        private static bool IsStandardListFile(string fileName)
        {
            ...
            if (!fileName.EndsWith(".unity3d", StringComparison.OrdinalIgnoreCase)) // Official list files only have numbers in them while all custom ones seem to have at least one letter, so this is enough
                return false;
