...
MsgExeNotFound=WARNING - HoneyCome files were not found in selected directory. This patch has to be installed directly to the main game directory in order to work properly.%n%nAre you sure that this directory is correct?
...
jp.MsgIncompatibleModsDetected=警告 - あなたのゲームフォルダで互換性のない改造が検出されました！ あなたは非常に古くなったモッズを使っているか、ハニカムがの他のゲームのモッズを使っています。 これを修正するには、古いモッズをすべて削除する必要があります。
jp.MsgExeNotFound=警告 - ハニカムが選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチをハニカムがのメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
MsgSteamUpdatesMissing=Your game is missing official updates from Steam. Please go to your Steam Library and update HoneyCome before installing this patch.
